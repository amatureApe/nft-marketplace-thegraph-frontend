Moralis.Cloud.afterSave("ItemListed", async (request) => {
  ...
  if (confirmed) {
    ...
    if (alreadyListedItem) {
      ...
      logger.info(`Deleted item with tokenId ${request.object.get(
        "tokenId"
...
Moralis.Cloud.afterSave("ItemCanceled", async (request) => {
  ...
  if (confirmed) {
    ...
    if (canceledItem) {
      ...
      logger.info(`Not item found with address ${request.object.get("address")} and tokenId: ${request.object.get("address")}`);
    }
  }
});
...
Moralis.Cloud.afterSave("ItemBought", async (request) => {
  ...
  if (confirmed) {
    const ActiveItem = Moralis.Object.extend("ActiveItem");
    const query = new Moralis.Query(ActiveItem);
    query.equalTo("marketplaceAddress", reqeust.object.get("address"));
    query.equalTo("nftAddress", request.object.get("nftAddress"));
    query.equalTo("tokenId", request.object.get("tokenId"));
    logger.info(`Marketplace | Query: ${query}`);
    ...
    if (boughtItem) {
      ...
      logger.info(`Deleted item with Token ID ${request.object.get(
        "tokenId"
      )} at address ${request.object.get("address")}`)
    } else {
      logger.info(`Not item found with address: ${request.object.get("address")} and tokenId: ${request.object.get("tokenId")}`);
    }
  }
})